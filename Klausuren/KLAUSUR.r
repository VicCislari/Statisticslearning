...
######aufgabe 3
x <-rawdata%>% filter(rawdata$�..country == "Germany")%>%
...
y<- rawdata %>% filter(rawdata$�..country == "Germany")%>%
            select(�..country,tests_done,new_cases,year_week)%>%
          subset(year_week > "2020-W42" & year_week < "2020-W54")%>%
                mutate(sum_of_tests = sum(tests_done), 
                   sum_of_newcases= sum(new_cases))%>%
              select(�..country,sum_of_tests,sum_of_newcases)%>%
            unique()
        
 
 
           rawdata %>% filter(rawdata$�..country == "Austria")%>%
            select(�..country,tests_done,new_cases)%>%
             subset(year_week > "2020-W42" & year_week < "2020-W54")%>%
          mutate(sum_of_tests = sum(tests_done), 
          sum_of_newcases= sum(new_cases))%>%
           select(�..country,sum_of_tests,sum_of_newcases)%>%
         unique()
           ...
           rawdata %>% filter(rawdata$�..country == "France")%>%
   select(�..country,tests_done,new_cases)%>%
             subset(year_week > "2020-W42" & year_week < "2020-W54")%>%
   mutate(sum_of_tests = sum(tests_done), 
          sum_of_newcases= sum(new_cases))%>%
 ...
 rawdata %>% filter(rawdata$�..country == "Italy")%>%
   select(�..country,tests_done,new_cases)%>%
   subset(year_week > "2020-W42" & year_week < "2020-W54")%>%
   mutate(sum_of_tests = sum(tests_done), 
          sum_of_newcases= sum(new_cases))%>%
 ...
 y<-raw_data%>%
   filter((�..country=="Germany" | �..country=="France" |
              �..country=="Austria" | �..country=="Italy") & year=="2020" & level=="national" & week<=52 &week>=48) %>% 
   group_by(�..country)%>%
  ...
  testing_rate_germany<- rawdata %>% filter(�..country=="Germany") %>% select(testing_rate)
min(testing_rate_germany)
max(testing_rate_germany)
quantile(testing_rate_germany$testing_rate, c(0.25,0.5,0.75))

ting_rate_france<- rawdata %>% filter(�..country=="France") %>% select(testing_rate)
