...
ml.est.N <- tibble(
  N = n.marked:50,
  est.1 = choose(n.marked,n.rd.1) * 
    choose(N-n.marked, n.caught.1-n.rd.1) /
    choose(N,n.caught.1),
  est.2 = est.1 * choose(n.marked+(n.caught.1-n.rd.1), n.rd.2) *
    choose(N-n.marked-(n.caught.1-n.rd.1),n.caught.2-n.rd.2) /
    choose(N,n.caught.2)
)
head(ml.est.N,20)
...
xtable(ml.est.N[1:15,1:2]) %>% 
  print(include.rownames = FALSE, floating = FALSE)
xtable(ml.est.N[3:15,c(1,2,3)]) %>% 
  print(include.rownames = FALSE, floating = FALSE, digits = c(0,4,4))
...
dev.copy2eps(file="d:/Lehre/MATHE/Statistik/Exercises/pictures/ml_est_red_deers.eps")
