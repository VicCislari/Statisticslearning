...
description: >
  Learn how to summarize the columns available in an R data frame with summarise(). You will also learn how to chain operations together with the pipe operator %>%, and how to compute grouped summaries using group_by()` together with summarise().
---
...
Use`min_rank(desc(avg_delay))` to rank `avg_delay` (for example) such that the largest delay receives rank one. 

Frontier airlines (`F9`) was the highest average departure delay.
...
This was a tricky one, but you can now see that `count()` is a handy short cut for `group_by()` + `summarise()` + `n()` (or `sum()`)."
